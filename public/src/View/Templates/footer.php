	<footer class="foot"><a href="https://github.com/Chung-Oh" target="_blank">&copy Daniel Chung</a></footer>
	<script type="module" src="../../app/js/nav-bar-button.js"></script>
	<script type="module" src="../../app/js/helpers/manipulate-form.js"></script>
	<script type="module" src="../../app/js/search-box.js"></script>
	<script type="module" src="../../app/js/form-category.js"></script>
	<script type="module" src="../../app/js/form-category-edit.js"></script>
	<script type="module" src="../../app/js/form-category-delete.js"></script>
	<script type="module" src="../../app/js/helpers/manipulate-table.js"></script>
	<script type="module" src="../../app/js/table-order.js"></script>
	<script type="module" src="../../app/js/msg-session.js"></script>
</body>
</html>